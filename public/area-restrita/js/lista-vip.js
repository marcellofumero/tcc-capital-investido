...
listavip.metodos = {
    ...
    listarTodos: async () => {        
        ...
        try{                                 
            await $.ajax({
                type: 'GET',
                url: comum.var.urlApiAreaRestrita + '/v1/listaVip/listaVip/',
                                ...
                                <tr>
                                    ...
                                    <td align="center">
                                        <div class="btnAcoesEditarExcluir">
                                            <a href="lista-vip-editar.html?id=${elem._id}" style="text-decoration:none !important">
                                                <i class="far fa-edit" title="Editar" style="cursor:pointer"></i>
                                            </a>
                                            &nbsp;
                                            <i class="far fa-trash-alt" data-toggle="modal" data-target="#modalExcluir" onclick="$('#msgExcluir').hide();$('#lblNomeListaTradeVipExcluir').text('${elem.titulo}');$('#txtListaTradeVipExcluir').val('${elem._id}')" title="Excluir" style="cursor:pointer" ></i>
                                        </div>
                                    </td>                          
                                </tr>
                            `); 
            ...
            comum.metodos.mensagemInformativa('msgListaTradeVip','Ocorreu um erro inesperado: ' + erro,'erro');
        }

        if (localStorage.getItem('PerfilUsuario') == "Administrador"){
            $('#btnCadastrarListaTradeVip').show();
            $('.btnAcoesEditarExcluir').show();
        }
        else{
            $('#btnCadastrarListaTradeVip').hide();
    ...
    obterDadosEdicao: () => {        
        try{                                 
            ...
            listavip.metodos.listarTipoInvestimento();
                                    
            $.ajax({
                type: 'GET',
                url: comum.var.urlApiAreaRestrita + '/v1/listaVip/listaVip/' + idListaTrade,
                beforeSend: (request) => { request.setRequestHeader("Authorization", "Bearer "+localStorage.getItem('TokenAcesso')); },
                success: function (response) { 
    ...
    excluir: (id) => {        
        try{                                  
            $.ajax({
                type: 'DELETE',
                url: comum.var.urlApiAreaRestrita + '/v1/listaVip/listaVip/' + id,
                beforeSend: (request) => { request.setRequestHeader("Authorization", "Bearer "+localStorage.getItem('TokenAcesso')); },
                success: function (response) { 
