...
noticia.metodos = {
    ...
    listarTodos: async () => {        
        ...
        try{                                 
            await $.ajax({
                type: 'GET',
                url: comum.var.urlApiAreaRestrita + '/v1/noticia/noticia/',
                                ...
                                <tr>
                                    ...
                                    <td align="center">
                                        <div class="btnAcoesEditarExcluir">
                                            <a href="noticia-editar.html?id=${elem._id}" style="text-decoration:none !important">
                                                <i class="far fa-edit" title="Editar" style="cursor:pointer"></i>
                                            </a>
                                            &nbsp;
                                            <i class="far fa-trash-alt" data-toggle="modal" data-target="#modalExcluir" onclick="$('#msgExcluir').hide();$('#lblNomeNoticiaExcluir').text('${elem.titulo}');$('#txtNoticiaExcluir').val('${elem._id}')" title="Excluir" style="cursor:pointer" ></i>
                                        </div>
                                    </td>                          
                                </tr>
                            `); 
            ...
            comum.metodos.mensagemInformativa('msgNoticia','Ocorreu um erro inesperado: ' + erro,'erro');
        }
    ...
    obterDadosEdicao: () => {        
        try{                                 
            var idNoticia = location.search.slice(1).split('=')[1];
                                                
            $.ajax({
                type: 'GET',
                url: comum.var.urlApiAreaRestrita + '/v1/noticia/noticia/' + idNoticia,
                beforeSend: (request) => { request.setRequestHeader("Authorization", "Bearer "+localStorage.getItem('TokenAcesso')); },
                success: function (response) { 
    ...
    excluir: (id) => {        
        try{                                  
            $.ajax({
                type: 'DELETE',
                url: comum.var.urlApiAreaRestrita + '/v1/noticia/noticia/' + id,
                beforeSend: (request) => { request.setRequestHeader("Authorization", "Bearer "+localStorage.getItem('TokenAcesso')); },
                success: function (response) { 
